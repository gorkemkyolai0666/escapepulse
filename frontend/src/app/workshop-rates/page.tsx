...
import {
  formatCurrency,
  formatWorkshopRateStatus,
  formatWorkshopCategory,
} from '@/lib/utils';

interface WorkshopRate {
  id: string;
  title: string;
  rateCategory: string;
  basePrice: number;
...
export default function WorkshopRatesPage() {
  const { token } = useAuth();
  const [tiers, setTiers] = useState<WorkshopRate[]>([]);
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await api.workshopRates.create(token, {
        title: form.title,
    ...
    <AppLayout>
      <div className="space-y-6">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <h1 className="font-display text-3xl text-primary">Hizmet Atölye Ücretileri</h1>
        ...
        {showForm && (
          <Card className="workstation-card">
            <CardHeader>
              <CardTitle className="font-display">Atölye Ücreti Kademesi Ekle</CardTitle>
            </CardHeader>
            <CardContent>
              <form onSubmit={handleCreate} className="grid gap-4 sm:grid-cols-2">
                ...
                <div className="space-y-2">
                  ...
                  >
                    {CATEGORIES.map((c) => (
                      <option key={c} value={c}>{formatWorkshopCategory(c)}</option>
                    ))}
                  </select>
                </div>
                <div className="space-y-2">
                  <Label htmlFor="basePrice">Temel Atölye Ücreti ($)</Label>
                  <Input
                    id="basePrice"
                    type="number"
                    min={0}
                ...
                <div className="space-y-2">
                  <Label htmlFor="priceMultiplier">Atölye Ücreti Çarpanı</Label>
                  <Input
                    id="priceMultiplier"
                    type="number"
                    min={0}
                ...
                <div className="space-y-2 sm:col-span-2">
                  ...
                  >
                    {STATUSES.map((s) => (
                      <option key={s} value={s}>{formatWorkshopRateStatus(s)}</option>
                    ))}
                  </select>
                </div>
        ...
        {!loading && !error && tiers.length === 0 && (
          <EmptyState
            title="Atölye Ücreti kademesi bulunamadı"
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-border bg-muted/50 text-left font-mono text-xs uppercase">
                      ...
                      <th className="p-3" scope="col">Temel Atölye Ücreti</th>
                  ...
                  <tbody>
                    {tiers.map((tier) => (
                      <tr key={tier.id} className="border-b border-muted hover:bg-muted/30">
                        ...
                        <td className="p-3">{formatWorkshopCategory(tier.rateCategory)}</td>
                        ...
                        <td className="p-3">
                          <Badge variant="secondary">{formatWorkshopRateStatus(tier.status)}</Badge>
                        </td>
                      </tr>
                    ))}
                  </tbody>
