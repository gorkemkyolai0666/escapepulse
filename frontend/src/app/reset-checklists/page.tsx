...
import {
  formatDateTime,
  formatResetChecklistStatus,
  formatResetChecklistCategory,
} from '@/lib/utils';
...
export default function ResetChecklistPage() {
  const { token } = useAuth();
  const [items, setItems] = useState<ResetChecklistItem[]>([]);
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await api.resetChecklist.create(token, {
        title: form.title,
    ...
    <AppLayout>
      <div className="space-y-6">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <h1 className="font-display text-3xl text-primary">Sıfırlama Kontrol Listeleri</h1>
        ...
        {showForm && (
          <Card className="mystery-card">
            <CardHeader>
              <CardTitle className="font-display">Sıfırlama Planı Ekle</CardTitle>
            </CardHeader>
            <CardContent>
              <form onSubmit={handleCreate} className="grid gap-4 sm:grid-cols-2">
                ...
                <div className="space-y-2">
                  ...
                  <select id="category" value={form.category} onChange={(e) => update('category', e.target.value)} className="flex h-10 w-full rounded-md border border-input bg-background px-3 text-sm">
                    {CATEGORIES.map((c) => (
                      <option key={c} value={c}>{formatResetChecklistCategory(c)}</option>
                    ))}
                  </select>
                </div>
                <div className="space-y-2">
                  <Label htmlFor="wing">Kanat / Bölüm</Label>
                  <Input id="wing" value={form.wing} onChange={(e) => update('wing', e.target.value)} placeholder="Örn: Basement Wing" />
                </div>
                <div className="space-y-2">
                ...
                <div className="space-y-2">
                  ...
                  <select id="status" value={form.status} onChange={(e) => update('status', e.target.value)} className="flex h-10 w-full rounded-md border border-input bg-background px-3 text-sm">
                    {STATUSES.map((s) => (
                      <option key={s} value={s}>{formatResetChecklistStatus(s)}</option>
                    ))}
                  </select>
                </div>
        ...
        {!loading && !error && items.length === 0 && (
          <EmptyState title="Sıfırlama planı yok" description="Henüz sıfırlama kontrol listesi eklenmemiş." />
        )}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {formatResetChecklistCategory(item.category)} · {item.wing || 'Genel'} · {formatDateTime(item.scheduledAt)}
                      </p>
                    </div>
                  </div>
                  <Badge variant="secondary">{formatResetChecklistStatus(item.status)}</Badge>
                </CardContent>
              </Card>
            ))}
          </div>
