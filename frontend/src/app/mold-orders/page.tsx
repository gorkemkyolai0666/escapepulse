...
import { formatMoldOrderStatus, formatCurrency } from '@/lib/utils';

interface MoldOrderItem {
  id: string;
  customerName: string;
  moldType: string;
  supplierName?: string;
...
export default function MoldOrdersPage() {
  const { token } = useAuth();
  const [items, setItems] = useState<MoldOrderItem[]>([]);
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await api.moldOrders.create(token, {
    ...
    <AppLayout>
      <div className="space-y-6">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <h1 className="font-display text-3xl text-primary">Kalıp Siparişleri</h1>
            <p className="text-muted-foreground">Kalıpit, mekanizma ve dekor sipariş takibi</p>
          </div>
        ...
        {showForm && (
          <Card className="workstation-card">
            <CardHeader>
              <CardTitle className="font-display">Kalıp Siparişi Ekle</CardTitle>
            </CardHeader>
            <CardContent>
              <form onSubmit={handleCreate} className="grid gap-4 sm:grid-cols-2">
                ...
                <div className="space-y-2 sm:col-span-2">
                  <Label htmlFor="moldType">Kalıp Kategorisi</Label>
                  <Input id="moldType" value={form.moldType} onChange={(e) => update('moldType', e.target.value)} required placeholder="Örn: İpek Saten Set" />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="price">Atölye Ücreti ($)</Label>
                ...
                <div className="space-y-2">
                  ...
                  <select id="status" value={form.status} onChange={(e) => update('status', e.target.value)} className="flex h-10 w-full rounded-md border border-input bg-background px-3 text-sm">
                    {STATUSES.map((s) => (
                      <option key={s} value={s}>{formatMoldOrderStatus(s)}</option>
                    ))}
                  </select>
                </div>
        ...
        {!loading && items.length > 0 && (
          <div className="grid gap-3 sm:grid-cols-2">
            {items.map((item) => (
              <Card key={item.id} className="workstation-card">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {item.moldType} · {item.supplierName || 'Tedarikçi belirtilmemiş'} · {formatCurrency(item.price)}
                      </p>
                    </div>
                  </div>
                  <Badge variant="secondary">{formatMoldOrderStatus(item.status)}</Badge>
                </CardContent>
              </Card>
            ))}
          </div>
