...
import {
  formatDateTime,
  formatCuringStatus,
  formatCuringCategory,
} from '@/lib/utils';

interface CuringChecklistItem {
  id: string;
  title: string;
  description?: string;
  category: string;
...
export default function CuringChecklistPage() {
  const { token } = useAuth();
  const [items, setItems] = useState<CuringChecklistItem[]>([]);
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await api.curingChecklist.create(token, {
        title: form.title,
              ...
              <form onSubmit={handleCreate} className="grid gap-4 sm:grid-cols-2">
                ...
                <div className="space-y-2">
                  ...
                  <select id="category" value={form.category} onChange={(e) => update('category', e.target.value)} className="flex h-10 w-full rounded-md border border-input bg-background px-3 text-sm">
                    {CATEGORIES.map((c) => (
                      <option key={c} value={c}>{formatCuringCategory(c)}</option>
                    ))}
                  </select>
                </div>
                <div className="space-y-2">
                ...
                <div className="space-y-2">
                  ...
                  <select id="status" value={form.status} onChange={(e) => update('status', e.target.value)} className="flex h-10 w-full rounded-md border border-input bg-background px-3 text-sm">
                    {STATUSES.map((s) => (
                      <option key={s} value={s}>{formatCuringStatus(s)}</option>
                    ))}
                  </select>
                </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {formatCuringCategory(item.category)} · {item.zone || 'Genel'} · {formatDateTime(item.scheduledAt)}
                      </p>
                    </div>
                  </div>
                  <Badge variant="secondary">{formatCuringStatus(item.status)}</Badge>
                </CardContent>
              </Card>
            ))}
          </div>
