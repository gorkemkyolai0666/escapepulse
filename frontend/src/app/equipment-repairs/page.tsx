...
export default function EquipmentRepairPage() {
  ...
  const load = () => {
    ...
    setLoading(true);
    setError(false);
    Promise.all([api.equipmentRepair.list(token), api.workstations.list(token)])
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await api.equipmentRepair.create(token, {
      ...
      <div className="space-y-6">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="text-muted-foreground">Kalıpit, mekanizma ve prop arıza kayıtları</p>
          </div>
            ...
            <CardContent>
              <form onSubmit={handleCreate} className="grid gap-4 sm:grid-cols-2">
                ...
                <div className="space-y-2 sm:col-span-2">
                  ...
                  <Input id="title" value={form.title} onChange={(e) => update('title', e.target.value)} required placeholder="Örn: RFID kalıpit arızası" />
                </div>
                <div className="space-y-2 sm:col-span-2">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {item.workstation?.name || 'İstasyon belirtilmemiş'} · {item.workstation?.zone} · {formatEquipmentRepairPriority(item.priority)} · {formatDateTime(item.reportedAt)}
                      </p>
                    </div>
                  </div>
