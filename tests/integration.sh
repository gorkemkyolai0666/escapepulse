...
echo "=== ResinPulse Integration Tests ==="
echo "API: $API_URL"
echo ""
...
LOGIN_RESPONSE=$(curl -s -w "\n%{http_code}" "$API_URL/auth/login" \
  ...
  -d '{"email":"demo@iridescentpourstudio.com","password":"demo123456"}')
...
HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" "$API_URL/pour-batches" -H "Authorization: Bearer $TOKEN")
...
HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" "$API_URL/curing-checklists" -H "Authorization: Bearer $TOKEN")
assert_status "List Quality Checklists" 200 "$HTTP_CODE"

HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" "$API_URL/mold-orders" -H "Authorization: Bearer $TOKEN")
assert_status "List Fabric Orders" 200 "$HTTP_CODE"

HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" "$API_URL/workshop-rates" -H "Authorization: Bearer $TOKEN")
assert_status "List Service Rates" 200 "$HTTP_CODE"

HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" "$API_URL/resin-studio" -H "Authorization: Bearer $TOKEN")
...
HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" "$API_URL/mold-orders/pending" -H "Authorization: Bearer $TOKEN")
assert_status "Pending Fabric Orders" 200 "$HTTP_CODE"

CREATE_ROOM=$(curl -s -w "\n%{http_code}" "$API_URL/workstations" \
  -H "Authorization: Bearer $TOKEN" \
  -H 'Content-Type: application/json' \
  -d '{"name":"Test Workstation #99","zone":"Test Zone","workstationType":"standard","notes":"Integration test workstation"}')
...
CREATE_PROP=$(curl -s -w "\n%{http_code}" "$API_URL/mold-orders" \
  -H "Authorization: Bearer $TOKEN" \
  -H 'Content-Type: application/json' \
  -d '{"customerName":"Laguna Clay","moldType":"B-Mix Cone 5","supplierName":"Laguna Clay","price":99}')
...
if [ -n "$PROP_ID" ]; then
  HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" "$API_URL/mold-orders/$PROP_ID" \
    -H "Authorization: Bearer $TOKEN" \
    -H 'Content-Type: application/json' \
    -X PATCH \
